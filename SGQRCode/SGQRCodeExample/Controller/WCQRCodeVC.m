...
//                [weakSelf presentViewController:vc animated:YES completion:nil];
                ...
                if(pp_code == 1){
                    ...
                    [sb instantiateViewControllerWithIdentifier:@"view2"];
                    lvc.modalPresentationStyle = 0;
                    [weakSelf presentViewController:lvc animated:NO completion:nil];
                }else{
                    UIStoryboard * sb =
                    [UIStoryboard storyboardWithName:@"Main" bundle:nil];
                    UIViewController * lvc =
                    [sb instantiateViewControllerWithIdentifier:@"view3"];
                    lvc.modalPresentationStyle = 0;
                    [weakSelf presentViewController:lvc animated:NO completion:nil];
                }
...
- (void)rightBarButtonItenAction {
    ...
    [obtain setBlockWithQRCodeObtainAlbumResult:^(SGQRCodeObtain *obtain, NSString *result) {
        ...
        if (result == nil) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [MBProgressHUD SG_hideHUDForView:weakSelf.view];
                [weakSelf presentViewController:jumpVC animated:YES completion:nil];
            });
        }
    }];
}
